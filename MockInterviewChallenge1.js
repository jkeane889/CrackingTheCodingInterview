...
*/
    ...
    const compressString = (str) => {
        ...
        for (let i = 0; i < str.length; i++) {
            ...
            /* If next character is different from current, than append this char
                and count to result string */
            if (i + 1 >= str.length || str.charAt(i) !== str.charAt(i + 1)) {
                compressString += "" + str.charAt(i) + consecutiveCount.toString();
                consecutiveCount = 0;
            };
        };

        return compressedString.length < str.length ? compressString : str;
    };
...
const findDirection = (ants, sides) => {
    /* Only two possible scenarios where they are all either moving LEFT or RIGHT */
    let totalPossibilities = Math.pow(2, sides); // total ants choosing either left or right (2 choices)
    let probabilityOfAvoidance = 2 / totalPossibilities;
    let probabilityOfCollision = totalPossibilities - probabilityOfAvoidance;  
    return probabilityOfCollision;
};
